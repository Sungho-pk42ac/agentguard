...
test('handleFindings echoes cveIds + cveSeverity after CVE enrichment (updateFindingCve)', async () => {
  const d = deps()
  const fp = 'a'.repeat(32)
  await d.storage.upsertFinding('orgA', 'pc1', rf({ fingerprint: fp }), 0)
  await d.storage.updateFindingCve('orgA', 'pc1', fp, ['GHSA-jf85-cpcp-j695', 'CVE-2019-10744'], 'critical')
  const res = await handleFindings('orgA', {}, d)
...
test('handleFindings echoes advisory:true for an advisory finding, and omits the fields when absent', async () => {
  const d = deps()
  await d.storage.upsertFinding('orgA', 'pc1', rf({ ruleId: 'mcp-unapproved', surface: 'mcp-risk', severity: 'low', advisory: true, fingerprint: 'b'.repeat(32) }), 0)
  await d.storage.upsertFinding('orgA', 'pc1', rf({ fingerprint: 'c'.repeat(32) }), 0) // plain finding, no cve/advisory
  const res = await handleFindings('orgA', {}, d)
