...
| Public reference | Borrow | Avoid | AgentGuard action |
|---|---|---|---|
| https://openai.github.io/openai-agents-python/guardrails/ | input/output/tool guardrail and tripwire vocabulary for when a workflow should stop or require review. | OpenAI Agents SDK integration, runtime tool interception, automatic guardrail enforcement, or OpenAI endorsement claim. | `scan-diff`, `scan-mcp`, `scan-log`, and SARIF evidence를 static tripwire-before-run approval question으로 바꿉니다. |
| https://docs.anthropic.com/en/docs/claude-code/security | permission-based architecture, read-only default, explicit permission, workspace trust, and human approval boundary language. | Anthropic approval, private Claude workspace access, sandbox guarantee, or Claude Code runtime control claim. | runner가 파일 편집·명령 실행·MCP tool을 열기 전에 같은 source artifact를 검사해 permission review owner에게 넘깁니다. |
| https://modelcontextprotocol.io/specification/2025-06-18/basic/security_best_practices | least privilege, consent, dangerous command highlighting, trust boundary, confused deputy, SSRF, token handling language for explaining why runner tool permissions need a boundary. | MCP conformance, runtime authorization, runtime sandbox, consent UI, OAuth/session enforcement claim. | `scan-mcp` evidence를 static pre-run approval question으로 만들고 `mcp.broad_filesystem_access` / `mcp.env_credential` findings를 권한 축소 조건에 연결합니다. |
...
| https://github.com/affaan-m/agentshield | public agent-security scanner category pressure around agent configurations, MCP servers, and tool permissions. | hosted app, GitHub App, ECC plugin, continuous monitoring, vendor-scale parity, or same-scope enterprise coverage claim. | AgentGuard의 차별점은 현재 repo fixture와 rerunnable command로 PR/MCP/transcript/SARIF source-of-record proof를 남기는 것이라고 좁혀 설명합니다. |
