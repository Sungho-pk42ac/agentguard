...
| GitHub SARIF code scanning — https://docs.github.com/en/code-security/code-scanning/integrating-with-code-scanning/uploading-a-sarif-file-to-github | SARIF rule/result/location artifact and reviewer-channel framing. | Do not imply native GitHub product status, automatic upload, or security-product scope. | Keep `agentguard scan-diff --sarif --out .agentguard-demo/agentguard.sarif` as machine-readable handoff for the same PR diff evidence. |
