...
| `PUBLIC_NOT_FOUND_404` | public URL returned HTTP 404 / not found from this environment. | record the stale or missing pointer and use only separately verified public sources. | borrowing evidence from the missing page, inferring hidden contents, or claiming the page was read. |
...
| MCP Security Best Practices — https://modelcontextprotocol.io/specification/draft/basic/security_best_practices | `PUBLIC_FETCH_200`; title observed: `Security Best Practices - Model Context Protocol`. | least privilege, explicit user consent, token boundary, sandboxing, and proxy isolation vocabulary for agent/tool rollout review. | runtime OAuth, consent UI, token revocation, sandbox enforcement, or MCP server hardening claim. | Route the signal to static `scan-mcp`/`scan-log` evidence, policy narrowing, and human approval-owner questions before rollout. |
...
| Snyk CLI agentic workflow docs — https://docs.snyk.io/scan-with-snyk/snyk-cli/scan-agentic-workflows | `PUBLIC_NOT_FOUND_404`; HTTP 404 from this run; no evidence should be borrowed from the missing Snyk docs URL. | only the provenance lesson: stale or missing public links must be labeled before they influence a slice. | claiming the missing page was read, using it as source evidence, or inferring Snyk feature coverage from it. | Use separately verified Snyk GitHub or registry signals only as category pressure, then route AgentGuard proof back to exact fixture-backed commands. |
...
| OpenAI AgentKit public page — https://openai.com/index/introducing-agentkit/ | `PUBLIC_WAF_403`; HTTP 403 from this Hermes environment; no AgentKit page content was read. | provenance lesson only: blocked public product pages must be labeled before influencing an AX slice. | claiming the AgentKit page was read, quoting hidden page content, or treating blocked content as insane-search evidence. | Use separately verified public references and exact AgentGuard evidence lanes instead of borrowing from the blocked page. |
| npm registry `agent-security-scanner-mcp` — https://registry.npmjs.org/agent-security-scanner-mcp | `PUBLIC_REGISTRY_FALLBACK_200`; public metadata observed `name=agent-security-scanner-mcp`, latest version `4.4.12`, description `npm audit for AI agents and MCP servers. Scan code, MCP tools, prompts, and hallucinated packages for Claude Code, Cursor, Windsurf, Cline, OpenClaw, and CI.` | MCP/agent scanner ecosystem category pressure, especially Claude Code/Cursor/CI workflow wording. | maturity, quality, install base, proof-layer endorsement, scanner parity, customer adoption, or replacement claim. | Differentiate AgentGuard as Korean-first rollout approval evidence across PR/MCP/transcript/SARIF, with registry metadata treated as public fallback only. |
