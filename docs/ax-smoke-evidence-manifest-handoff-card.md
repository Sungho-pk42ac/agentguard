...
- no JSON/SARIF field name change: `checks[]`, `surface`, `command`, `exitCode`, `acceptedNonZero`, `acceptedNonZeroReason`, `startedAt`, `completedAt`, `durationMs`, `artifact`, `ruleIds`, SARIF `ruleId`는 machine-facing contract다.
