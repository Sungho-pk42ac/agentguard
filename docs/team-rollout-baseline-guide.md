...
`.agentguard-demo/findings.json`는 reviewer/CI가 finding 배열을 기계적으로 파싱하기 위한 JSON artifact 경로입니다. `.agentguard-demo/agentguard.sarif`는 GitHub code scanning handoff를 로컬에서 확인하기 위한 SARIF artifact 경로입니다. risky 입력은 BLOCK으로 non-zero exit가 날 수 있음.
...
- JSON: `.agentguard-demo/findings.json` 또는 GitHub Action의 `json-path` output.
