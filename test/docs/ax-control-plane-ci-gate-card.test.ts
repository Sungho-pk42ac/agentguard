...
const docPath = join(repoRoot, 'docs', 'ax-control-plane-ci-gate-card.md')
...
test('AX control-plane CI gate card exists and is linked from README and examples docs', () => {
  assert.ok(existsSync(docPath), 'docs/ax-control-plane-ci-gate-card.md should exist')

  const rootReadme = readFileSync(join(repoRoot, 'README.md'), 'utf8')
  const examplesDoc = readFileSync(join(repoRoot, 'docs', 'examples.md'), 'utf8')

  assert.match(rootReadme, /\[AX control-plane CI gate card\]\(docs\/ax-control-plane-ci-gate-card\.md\)/)
  assert.match(examplesDoc, /\[AX control-plane CI gate card\]\(ax-control-plane-ci-gate-card\.md\)/)
})

test('AX control-plane CI gate card is Korean-first and covers required headings', () => {
  const doc = readDoc()
  let previousIndex = -1
  ...
  assert.match(
    doc,
    /\|\s*Company problem signal\s*\|\s*AgentGuard surface\s*\|\s*Control-plane \/ CI gate\s*\|\s*Exact evidence command \/ artifact\s*\|\s*Approver decision\s*\|\s*Rerun \/ freshness trigger\s*\|/,
  )

  for (const surface of ['PR diff', 'MCP config', 'transcript/log', 'SARIF/report', 'smoke/evidence freshness']) {
    expectLiteral(doc, surface)
  }
})

test('AX control-plane CI gate card uses exact fixture-backed commands and existing fixture paths', () => {
  const doc = readDoc()
  ...
  expectLiteral(doc, '.agentguard-demo/control-plane-ci-gate.sarif')
  expectLiteral(doc, '.agentguard-demo/ax-evidence-smoke/manifest.json')
})

test('AX control-plane CI gate card includes public reference rows with borrow and avoid guardrails', () => {
  const doc = readDoc()

  assert.match(doc, /\|\s*Public reference\s*\|\s*Borrow\s*\|\s*Avoid\s*\|\s*AgentGuard control-plane CI use\s*\|/)
  ...
  assert.match(doc, /authorization|token\/session boundary|least privilege/i)
  assert.match(doc, /SARIF `result`|ruleId|artifactLocation|region.startLine/i)
  assert.match(doc, /certification|runtime OAuth|automatic upload|replacement/i)
})

test('AX control-plane CI gate card preserves English-compatible machine contracts', () => {
  const doc = readDoc()
...
test('AX control-plane CI gate card bans unsupported adoption certification parity and runtime-auth claims', () => {
  const doc = readDoc()
