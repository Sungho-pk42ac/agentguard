...
const docPath = join(repoRoot, 'docs', 'ax-evidence-command-failure-triage.md')
...
const fixtureBackedCommands = [
  ...
  {
    command: 'node dist/index.js scan-diff --sarif --out .agentguard-demo/evidence-command-failure-triage.sarif < examples/risky-pr.diff',
    fixtures: ['examples/risky-pr.diff'],
  },
  {
...
test('AX evidence command failure triage card exists and is linked from public docs', () => {
  assert.ok(existsSync(docPath), 'docs/ax-evidence-command-failure-triage.md should exist')

  const rootReadme = readFileSync(join(repoRoot, 'README.md'), 'utf8')
  const examplesDoc = readFileSync(join(repoRoot, 'docs', 'examples.md'), 'utf8')

  assert.match(rootReadme, /\[AX evidence command failure triage card\]\(docs\/ax-evidence-command-failure-triage\.md\)/)
  assert.match(examplesDoc, /\[AX evidence command failure triage card\]\(ax-evidence-command-failure-triage\.md\)/)
})

test('AX evidence command failure triage card has Korean-first triage sections', () => {
  const doc = readDoc()
  ...
  for (const term of [
    '한국어 우선',
    'AX Rollout Guard',
    'expected risky nonzero',
    'build/setup failure',
    'stale/mixed artifact',
  ...
  assert.match(doc, /non-zero[\s\S]{0,500}verdict[\s\S]{0,500}artifact path/i)
  assert.match(doc, /SARIF[\s\S]{0,500}artifact[\s\S]{0,500}approval/i)
})

test('AX evidence command failure triage card uses exact fixture-backed commands with existing paths', () => {
  const doc = readDoc()
...
test('AX evidence command failure triage card cites public references with borrow avoid action rows', () => {
  const doc = readDoc()
  ...
  assert.match(doc, /MCP Security Best Practices[\s\S]{0,900}runtime OAuth/i)
...
test('AX evidence command failure triage card preserves machine contracts and bans fake claims', () => {
  const doc = readDoc()
  ...
  const proseWithoutGuardrailBullets = doc
    ...
    .filter((line) => {
      ...
      if (/^\|\s*(?:OWASP|MCP Security|GitHub SARIF|Snyk agent-scan|Tencent AI-Infra-Guard|splx agentic-radar)\b/.test(line)) return false
      return true
    })
    .join('\n')
  ...
  assert.doesNotMatch(proseWithoutGuardrailBullets, /(?:OWASP|GitHub|SARIF|Snyk|Tencent|splx-ai|agentic-radar)[^\r\n|.]{0,80}(?:공식\s*검증|검증\s*완료|인증\s*완료|approved|verified|replacement|parity|동등)/i)
