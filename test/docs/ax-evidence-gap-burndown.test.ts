...
const docPath = join(repoRoot, 'docs', 'ax-evidence-gap-burndown.md')
...
test('AX evidence gap burndown exists and is linked from examples', () => {
  assert.ok(existsSync(docPath), 'docs/ax-evidence-gap-burndown.md should exist')
  const examplesDoc = readFileSync(join(repoRoot, 'docs', 'examples.md'), 'utf8')
  assert.match(examplesDoc, /\[AX evidence gap burndown\]\(ax-evidence-gap-burndown\.md\)/)
})

test('AX evidence gap burndown maps public reference pressure to the next proof artifact', () => {
  const doc = readDoc()
  ...
  assert.match(doc, /\|\s*Gap\s*\|\s*Why it matters for AX judging\s*\|\s*Next proof artifact\s*\|\s*Evidence command\s*\|\s*Owner decision\s*\|/)
  assert.match(doc, /대상권|unknown company problem|AX judging/i)
  ...
  for (const phrase of [
    ...
    'public scanner category pressure',
    'source-of-record',
    'fresh-clone rerun',
    'approval owner',
    'residual-risk note',
...
test('AX evidence gap burndown uses exact fixture-backed commands with existing inputs', () => {
  const doc = readDoc()
...
test('AX evidence gap burndown preserves machine contracts and avoids fake claims', () => {
  const doc = readDoc()
  ...
  const claimableLines = doc
    .split('\n')
    .filter(
      (line) =>
        !line.includes('Avoid:') &&
        !/^\s*-\s*No\b/.test(line) &&
        !line.includes('Non-claim guardrails'),
    )
    .join('\n')
  ...
  for (const forbidden of [
    ...
    assert.doesNotMatch(claimableLines, forbidden)
  }
})
